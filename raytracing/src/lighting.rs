...
// En el Nether: rojo distorcionado con neblina
pub fn sky(dir: Vec3, is_nether: bool) -> Vec3 {
    if is_nether {
        // Cielo del Nether: rojo oscuro suave con distorsión
        let t = dir.y.clamp(-1.0, 1.0);
        let noise = (dir.x * 3.5).sin() * (dir.z * 4.2).cos() * 0.12;
        let daytime = 0.5 * (t + 1.0) + noise;
        
        let horizon = Vec3::new(0.45, 0.12, 0.12); // Rojo suave en el horizonte
        let zenith = Vec3::new(0.25, 0.05, 0.05);   // Rojo muy oscuro arriba
        
        zenith.mul(daytime.clamp(0.0, 1.0)).add(horizon.mul((1.0 - daytime).clamp(0.0, 1.0)))
    } else {
        // Cielo del Overworld: azul normal
        let t = dir.y.clamp(-1.0, 1.0);
